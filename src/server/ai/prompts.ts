...
- MCQ: exactly 4 options with keys A, B, C, D. **The prompt is the question stem ONLY — never list the options inside the prompt text.** The four options go solely in the options field; the reader sees them rendered as selectable choices below the prompt, so an option list written into the prompt too shows up twice. If your prompt contains "A)", "B)", or a run of the choices, move them out — the prompt should end at the question mark. (A fenced code block in the prompt is fine and often needed; an inlined answer list is not.)
- MCQ: exactly one option is correct, and the other three must be **unambiguously wrong** — each states something false. A distractor that is *true*, or that is a correct-but-less-detailed version of the right answer, breaks the question: the student picks it, is marked wrong, and is right to be angry. The correct option must be the only defensible one, not merely the most complete or best-worded. Before finalising, check every distractor and ask "could a knowledgeable person argue this is also correct?" — if yes, rewrite it so the answer is false. Distractors must still be plausible and represent real misconceptions — never filler, never joke options, never "all of the above".
